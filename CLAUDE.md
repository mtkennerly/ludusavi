...
`desktop/` is a separate Tauri project (React+TS, `pnpm`) that depends on this crate as a path dependency with `default-features = false` (see `desktop/src-tauri/Cargo.toml`) — no `app` feature, so no clap/rfd/dialoguer pulled into the GUI backend. Its `#[tauri::command]` handlers (`desktop/src-tauri/src/lib.rs`) wrap `api.rs::Ludusavi` directly, the same struct the CLI's `sync` subcommand uses — extend that struct's methods rather than reaching into `sync.rs`/`cloud.rs` from Tauri code. Run it with `cd desktop && pnpm install && pnpm run tauri:dev` (or `pnpm tauri dev`; the former works around a WebKitGTK/Wayland crash, see `desktop/scripts/dev.sh`). Building a real binary: see README.md's "Building" section. See `AGENTS.md`'s "Frontend Pivot" section for the full sequencing/rationale.

The frontend (`desktop/src/App.tsx`, `CloudSettings.tsx`) is two screens, no router lib — plain `useState<'sync'|'settings'>` in `App.tsx`, gear icon toggles. Sync screen: single unified game list, search box (debounced `search_games`, capped at 50, only queries when non-empty — never dumps the ~19k-title manifest by default) plus a "Scan" button (`scan_games`, a full-library `Finality::Preview` backup — same cost/shape as upstream's startup scan) both feed into the same list; a star toggle (`set_game_enabled`) controls both membership in `sync.enabled_games` and sort-to-top. Only starred rows show Backup/Push/Pull/Status buttons.
...
- `sync.rs` — fork's per-game push/pull/merge logic (`push_game`, `pull_game`, `get_game_sync_info`, `merge_cloud_sync_state`, `local_wine_prefix`). Callers: `cli.rs`'s `sync push|pull|status` subcommand and `api.rs`'s `Ludusavi::sync_push`/`sync_pull`/`sync_status` wrappers.
- `scan/semantic/discovery.rs` — `WineEnvironment::prefixes_for_game`, this-machine prefix resolution for restore (registry → config → Steam compatdata → launchers). See "Cross-device Wine/Proton remap" below.
- `api.rs` — `Ludusavi` struct, the stable library entry point: `back_up`, `restore`, `list_backups`, `edit_backup`, `find_title`, `search_games`/`set_game_enabled` (game picker), `sync_push`/`sync_pull`/`sync_status`, `wine_prefixes_for`/`backup_wine_prefixes`/`registered_prefixes` (Wine remap diagnostics), `cloud_status`/`set_cloud_remote_google_drive`/`disconnect_cloud_remote`/`set_cloud_path`/`set_cloud_synchronize`. This is the surface `desktop/`'s Tauri backend binds against directly, rather than reaching into `cli.rs` or `sync.rs`.
...
2. `resource/sync_state.rs` — `SyncStateFile` (`settings.config`, JSON, at the backup root) maps game name → `GameSyncEntry { last_push, device, mapping_path, prefixes }`. Written atomically (temp + rename). It rides along in the same rclone transfer as the game files.
3. **`SyncStateFile::merge_from` is load-bearing — never overwrite `settings.config` wholesale.** `sync.rs::merge_cloud_sync_state`/`fetch_cloud_sync_state` download the cloud's copy to a scratch dir (`.ludusavi-cloud-state`), then merge it into the local file rather than replacing it: newer `last_push` wins per game, but `prefixes` maps are *always unioned* — each device only ever writes its own key, so a plain overwrite would erase every other device's recorded prefix on the very next pull. `push_game` merges-then-writes too, so pushing can't drop entries a device has never seen.
...
Same game, same cloud folder, different machines: the Linux username and the Steam Proton compatdata app ID (`steamapps/compatdata/<id>/pfx`) both differ per device — even for the *same* game, since a non-Steam-shortcut app ID is generated per machine. Without remapping, pulling the Deck's save onto a PC and restoring writes to a literal `/home/deck/...` path. Full write-up: `/home/kookoo/.claude/plans/delegated-wobbling-mochi.md`.

Two separate mechanisms, deliberately kept apart:

- **Decompose** (`layout.rs::BackupSemantics`, per backup) — records which absolute-path prefix in *this* backup was a Wine/Proton prefix. Written at backup time in `scan.rs::wine_prefixes_with_included_files` (now includes Steam compatdata candidates, not just custom-game/launcher prefixes). Read at restore via `BackupSemantics::merged(full, diff)` (full+diff must be unioned — an old full backup with no semantics plus a newer diff that has them would otherwise leave every *inherited* file unmapped) and matched longest-prefix-first in `semantic::generate_restore_redirect`.
- **Recompose** (`GameSyncEntry.prefixes: BTreeMap<device, path>` in `settings.config`, per device, cross-device) — where *this* machine should write. Populated by `sync.rs::push_game` reading the prefix straight off the backup it just made (`sync.rs::local_wine_prefix`); read at restore via `scan/semantic/discovery.rs::WineEnvironment::prefixes_for_game`, priority: `--wine-prefix` CLI arg → registry (`prefixes[current_device()]`) → `custom_games[].wine_prefix` → manifest `wine_prefix` → Steam compatdata discovery (shortcut ID before manifest Steam ID) → Heroic/Lutris launcher prefix. Every candidate goes through `Prefix::validated`, so a stale registry entry self-heals by falling through.

`semantic::Wine` holds `prefixes: Vec<Prefix>` (was a single `preferred_prefix: Option<Prefix>`) — with more than one validated candidate, `pick_prefix` in `semantic.rs` prefers whichever already has the game's save folder on disk. `Wine::for_game(name, &WineEnvironment)` replaces the old `Wine::for_game(name, &Config)` signature at all four backup/restore call sites (`api.rs:~200,~390`, `cli.rs:~310,~610`).

`config.scan.redirect_wine` now **defaults `true`** (was `false`) — verified backwards-safe: an old backup with empty semantics restoring on Linux falls through every branch in `generate_restore_redirect` to `None` (no redirect), identical to today. Restore **refuses** (`Error::WinePrefixNotFound`, `lang.rs`/`en-US.ftl`) rather than silently writing a foreign path when a backup needs a Wine prefix and none is found locally — don't relax this to a warning; that guardrail is the point.

`api.rs::Ludusavi` exposes `wine_prefixes_for`/`backup_wine_prefixes`/`registered_prefixes` as read-only diagnostics for a frontend to warn before a restore would fail — not yet wired into `desktop/`'s UI.
