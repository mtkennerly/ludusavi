...
unable-to-synchronize-with-cloud = クラウドと同期できません。
cloud-synchronize-conflict = ローカルとクラウドのバックアップが競合しています。これを解決するには、アップロードまたはダウンロードを実行してください。
...
label-custom = Custom
label-none = なし
label-change-count = 変更点: { $total }
label-unscanned = 未スキャン
...
new-version-check = アプリケーションの更新を自動的に確認する
